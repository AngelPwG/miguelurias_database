...
const EditarPersonaPage = () => {
        ...
        const cargarDatos = async () => {
            try {
                ...
                setPersonaData({
                    ...
                    cumple: persona.cumple ? persona.cumple.split('T')[0] : '', // YYYY-MM-DD
                    telefono: persona.telefono || '',
                    direccion: persona.direccion || '',
                    estado: persona.estado || 'activo',
                    loreGeneral: persona.loreGeneral || ''
                });

                // Cargar Relaciones Existentes
                if (persona.relaciones) {
                    setRelaciones(persona.relaciones.map(r => ({
                        personaDestinoId: r.personaDestinoId,
                        personaDestinoNombre: r.personaDestinoNombre
                    })));
                }

                // Cargar Grupos Existentes
                if (persona.grupos) {
                    setGruposSeleccionados(persona.grupos.map(g => g.id));
                }

                // Cargar Eventos Existentes
                if (persona.eventos) {
                    setEventosSeleccionados(persona.eventos.map(e => ({
                        eventoId: e.id,
                        nombreEvento: e.nombre,
                        rol: e.rol || '' // Si el backend no devuelve rol aun, sera vacio
                    })));
                }

                // 2. Obtener Artículo
                if (persona.articuloId) {
                    const articulo = await obtenerArticuloPorId(persona.articuloId);

                    setArticuloData({
                        titulo: articulo.titulo || '',
                        tipo: articulo.tipo || 'persona',
                        secciones: articulo.secciones?.map(s => ({
                            ...s,
                    ...
                    if (articulo.galeria) {
                        setIdsExistentes(articulo.galeria.map(m => m.id));
                        setGaleriaPreviews(articulo.galeria.map(m => m.url));
                    }
                }
            } catch (err) {
    ...
    const handleArticuloChange = (e) => {
        const { name, value } = e.target;
        setArticuloData(prev => ({ ...prev, [name]: value }));
    };
    ...
    const removeGaleriaImage = (index) => {
        if (index < idsExistentes.length) {
            const nuevosIds = idsExistentes.filter((_, i) => i !== index);
            setIdsExistentes(nuevosIds);
        } else {
            const fileIndex = index - idsExistentes.length;
    ...
    const agregarSeccion = () => {
        ...
        setArticuloData(prev => ({ ...prev, secciones: [...prev.secciones, nuevaSeccion] }));
    };
    ...
    const eliminarSeccion = (index) => {
        ...
        const seccionesReordenadas = nuevasSecciones.map((seccion, i) => ({ ...seccion, orden: i + 1 }));
        setArticuloData(prev => ({ ...prev, secciones: seccionesReordenadas }));
    };
    ...
    const moverSeccion = (index, direccion) => {
        ...
        [nuevasSecciones[index], nuevasSecciones[nuevoIndex]] = [nuevasSecciones[nuevoIndex], nuevasSecciones[index]];
        const seccionesReordenadas = nuevasSecciones.map((seccion, i) => ({ ...seccion, orden: i + 1 }));
        setArticuloData(prev => ({ ...prev, secciones: seccionesReordenadas }));
    };
    ...
    const handleAgregarRelacion = () => {
        ...
        // Evitar duplicados y auto-referencia
        if (Number(selectedPersonaId) === Number(id)) {
            alert("No puedes agregar una relación con la misma persona.");
            return;
        }
        // Check both string/number types just to be safe
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (galeriaFiles.length > 0) {
                nuevosIdsGaleria = await subirMultiplesArchivos(galeriaFiles, 'imagen');
            }
            ...
            const seccionesProcesadas = await Promise.all(
                articuloData.secciones.map(async (seccion) => {
                    ...
                    else if (seccion.tipo === 'imagen' && !seccion.archivoImagen && seccion.multimedia) {
                        return {
                            tipo: seccion.tipo,
                            titulo: seccion.titulo,
                            multimediaId: seccion.multimedia.id,
                            orden: seccion.orden,
                            nivel: seccion.nivel
                        };
                    }
            ...
            // 3. Combinar IDs de Galería
            const idsDeSecciones = seccionesProcesadas
                .filter(s => s.tipo === 'imagen' && s.multimediaId)
                .map(s => s.multimediaId);

            const galeriaFinal = [...idsExistentes, ...nuevosIdsGaleria, ...idsDeSecciones];
            const galeriaUnica = [...new Set(galeriaFinal)];

            // 4. Preparar Update Objects
            ...
            // 5. Enviar Update
            await actualizarPersona(id, personaUpdate, articuloUpdate);
            alert('✅ Persona actualizada exitosamente');
            navigate(`/persona/${id}`);

        } catch (err) {
            console.error("Error al actualizar:", err);
            setError("Error al guardar cambios. Revisa la consola.");
        } finally {
            setSubmitting(false);
        }
    };
        ...
        <div className="min-h-screen bg-wiki-bg text-wiki-text p-4 md:p-8 font-mono">
            ...
            <header className="mb-8 border-b border-wiki-border pb-6 flex justify-between items-center">
                <div>
                    <h1 className="text-3xl md:text-4xl font-bold tracking-tighter text-white">
                        EDITAR <span className="text-wiki-accent">PERSONA</span>
                    </h1>
                </div>
                <button
            ...
            <div className="max-w-4xl mx-auto">
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-wiki-block border border-wiki-border rounded-lg p-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" name="nombre" value={personaData.nombre} onChange={handlePersonaChange} required
                            ...
                            <div>
                                ...
                                <input type="text" name="apodos" value={personaData.apodos} onChange={handlePersonaChange}
                                    className="w-full bg-wiki-bg border border-wiki-border rounded px-3 py-2 text-white focus:border-wiki-accent focus:outline-none" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold mb-2">Cumpleaños</label>
                                <input type="date" name="cumple" value={personaData.cumple} onChange={handlePersonaChange}
                                    className="w-full bg-wiki-bg border border-wiki-border rounded px-3 py-2 text-white focus:border-wiki-accent focus:outline-none" />
                            </div>
                            <div>
                                <label className="block text-sm font-bold mb-2">Teléfono</label>
                                <input type="text" name="telefono" value={personaData.telefono} onChange={handlePersonaChange}
                                    className="w-full bg-wiki-bg border border-wiki-border rounded px-3 py-2 text-white focus:border-wiki-accent focus:outline-none" />
                            </div>
                            <div>
                                ...
                                <input
                                    ...
                                    className="w-full bg-wiki-bg border border-wiki-border rounded px-3 py-2 text-white focus:border-wiki-accent focus:outline-none"
                                    placeholder="Ej: Activo, Desaparecido"
                                />
                            </div>
                            <div className="md:col-span-2">
                                <label className="block text-sm font-bold mb-2">Dirección</label>
                                <input type="text" name="direccion" value={personaData.direccion} onChange={handlePersonaChange}
                                    className="w-full bg-wiki-bg border border-wiki-border rounded px-3 py-2 text-white focus:border-wiki-accent focus:outline-none" />
                            </div>
                            <div className="md:col-span-2">
                                <label className="block text-sm font-bold mb-2">Lore / Descripción</label>
                                <textarea name="loreGeneral" value={personaData.loreGeneral} onChange={handlePersonaChange} rows="3"
                                    className="w-full bg-wiki-bg border border-wiki-border rounded px-3 py-2 text-white focus:border-wiki-accent focus:outline-none" />
                            </div>
                        </div>
                    </div>
                    ...
                    {/* [02] Galería */}
                    <div className="bg-wiki-block border border-wiki-border rounded-lg p-6">
                        <h2 className="text-xl font-bold text-wiki-accent mb-4">[02] GALERÍA</h2>
                        <div className="space-y-4">
                            <div className="border-2 border-dashed border-wiki-border rounded-lg p-6 text-center hover:border-wiki-accent transition-colors">
                                <label className="cursor-pointer block">
                                    ...
                                    <span className="font-bold">Agregar fotos nuevas</span>
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                                    {galeriaPreviews.map((src, index) => (
                                        <div key={index} className="relative group">
                                            <img src={src} alt="Preview" className="w-full h-24 object-cover rounded border border-wiki-border" />
                                            <button type="button" onClick={() => removeGaleriaImage(index)}
                    ...
                    <div className="flex gap-4 justify-end">
                        <button type="button" onClick={() => navigate(`/persona/${id}`)} className="px-6 py-2 border border-wiki-border hover:bg-wiki-border transition-colors">
                            CANCELAR
                        </button>
                        <button type="submit" className="px-6 py-2 bg-wiki-accent text-black font-bold hover:bg-green-400 transition-colors" disabled={submitting}>
